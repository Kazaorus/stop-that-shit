...
活动并发量。Stop That Shit 在受覆盖的 Hook 路径上检查这些明确边界。Agent 仍然可以读
...
`Number.MAX_SAFE_INTEGER`，设置为 `0` 会禁止 delegation；一个 batch 超出限制时
整批拒绝，不排队也不部分执行。明确确认同步完成的 `action.after`、subagent stop
或 session end 会释放活动槽位；后台或状态未知的调用会保守保留到明确的生命周期
事件。适配器不会按事件到达顺序猜测 reservation 的归属。旧 schema 中的有效
...
| 添加依赖 | 询问 | `deps=allow` |
| 启动 subagent | 超出活动并发上限时停止 | `agents=N` |

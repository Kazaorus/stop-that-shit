...
test('doctor, runtime, explain, and label expose the local evidence chain', (t) => {
  ...
  const event = recordDecision({
    ...
    contract: { mode: 'review', level: 'guard', totalAgentBudget: Number.MAX_SAFE_INTEGER, concurrentAgentBudget: Number.MAX_SAFE_INTEGER, hashPolicy: 'deny', dependencyPolicy: 'ask', allowedPaths: [] },
